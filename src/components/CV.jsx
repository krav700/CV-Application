...
function CV({ person }) {
    ...
    return (
        <div className='cv'>
            <div className='links'>
                <button className='pdf-button link' onClick={() => {downloadPDF()}}>    
                    <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width="35px">
                ...
                <button className='print-button link' onClick={() => {printCV()}}>    
                    <svg data-slot="icon" fill="none" width="35px" strokeWidth="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            {person.practicalExp.map((work) => 
                <Activity activity={work} person={person} key={work.id}/>
            )}

            <h2 className='section-name'>EDUCATION</h2>
            {person.schools.map((school) => 
                <Activity activity={school} person={person} key={school.id}/>
            )}
